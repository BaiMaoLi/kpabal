...
class dating extends CI_Controller {
    ...
    public function index($category = "")
    {
        ...
        $header_data = $this->__generate_header_data("dating");
        $footer_data = $this->__generate_footer_data();

        $data['rents']=$this->dating_model->get_recent_rent();
        $data['sales']=$this->dating_model->get_recent_sale();
        $data['prents']=$this->dating_model->get_premium_rent();
        $data['psales']=$this->dating_model->get_premium_sale();
        $data['cats1']=$this->dating_model->get_cat(3);
        $data['cats2']=$this->dating_model->get_cat(9);
        $data['locs']=$this->dating_model->get_locations();
        $data['brokers']=$this->dating_model->get_bocker();

        $this->load->view('kpabal/dating/header',$header_data);
        $this->load->view('frontend/dating/index',$data);
    ...
    public function search_result($page = "")
    {
        ...
        if($page<1)$page=1;
        $data['rents']=$this->dating_model->get_search_data($search_data,$page);

        $data1['page']=$page;
        $data['cats1']=$this->dating_model->get_cat(3);
        $data['cats2']=$this->dating_model->get_cat(9);
        $data['locs']=$this->dating_model->get_locations();
        $data['key']=$this->dating_model->get_gmapi();
        $data['lat']=40.75233;
        $data['lng']=-73.85346;
        $loc_data=$this->dating_model->get_locations($data['search_data']['location']);
        $zip=$loc_data[0]['zip'];
        if(!empty($zip)) {
            //Formatted address
        ...
        $this->load->view('kpabal/dating/header',$header_data);
        $this->load->view('frontend/dating/result',$data);
        $this->load->view('kpabal/common/footer',$footer_data);
        $this->load->view('frontend/dating/result_js',$data1);
    }

    public function propertyimageUpload(){		
	if($_FILES["file"]["name"]!=""){     
	$target_dir = 'assets/dating_logos/';       
	...
	$id=$this->dating_model->insertData('tbl_dating_images',$input);	
	$ret="<img src='".base_url().$target_file."' width='200px'><input type='hidden' name='pimgs[]' value='$id'>";		
	echo $ret;      
	}		
	}    
	public function listProperty(){	
$uid=$this->dating_model->get_current_user();
	if($uid!=null){	
	$data['selected'] = 'hosing';
	$header_data = $this->__generate_header_data("dating");      
	$footer_data = $this->__generate_footer_data();     
	...
	$this->load->view('frontend/dating/list',$data);   
	$this->load->view('kpabal/common/footer',$footer_data);      
	$this->load->view('frontend/dating/result_js',$data1);	
}else{
	...
	$uid=$this->dating_model->get_current_user();
	 $data['authorID']=$this->dating_model->get_current_user();
	if($uid!=null){
	$header_data = $this->__generate_header_data("dating");   
	$footer_data = $this->__generate_footer_data();            
    $uid=$this->dating_model->get_current_user();
	...
	$data['usermaster']=$this->dating_model->getData('tbl_member',array('memberIdx'=>$uid));
	$data['userdata']=$this->dating_model->getData('tbl_dating_broker',array('author'=>$uid));
	$data['properties']=$this->dating_model->get_all_properties($uid);
	$this->load->view('kpabal/common/header',$header_data);      
	$this->load->view('frontend/dating/dashboard',$data);      
	$this->load->view('kpabal/common/footer',$footer_data);     
	$this->load->view('frontend/dating/result_js');	
	}else{
		redirect(base_url().'login');
	}
	}  
	public function details($id = "")
    {
        ...
        $data['data']=$this->dating_model->get_recent_rent($id);	
		
		$pics= json_decode($data['data'][0]['images']);	
		$allpic=array();			
		if($pics!=null){
		foreach($pics->images as $im){	
		$allpic[]=$this->dating_model->getData('tbl_dating_images',array('id'=>$im))[0]['src'];		
		}			
		}
		$data['images']=$allpic;
		
       /* $data['author']=$this->dating_model->get_user_name($data['data'][0]['author']);
        $data['authorEmail']=$this->dating_model->get_user_email($data['data'][0]['author']);
        $data['authorID']=$data['data'][0]['author'];
        $data['location']=$this->dating_model->get_location_name($data['data'][0]['location']);
        $data['category']=$this->dating_model->get_cat_name($data['data'][0]['subcat']);*/
        $data['cats1']=$this->dating_model->get_cat(3);
        $data['cats2']=$this->dating_model->get_cat(9);
        $data['locs']=$this->dating_model->get_locations();
        $data['current_user']=$this->dating_model->get_current_user();
        $data['authorID']=$this->dating_model->get_current_user();


        $this->db->set("views",$data['data'][0]["views"]+1);
        $this->db->where("id",$id);
        $this->db->update("dating");

        $this->load->view('kpabal/common/header',$header_data);
        $this->load->view('frontend/dating/rentDetail',$data);
        $this->load->view('kpabal/common/footer',$footer_data);
        $this->load->view('frontend/dating/result_js');
    }
    ...
    public function saleDetail($id = "")
    {
        ...
        $data['data']=$this->dating_model->get_recent_sale($id);
        ...
        $this->db->set("views",$data['data'][0]["views"]+1);
        $this->db->where("id",$id);
        $this->db->update("dating_sale");

        $this->load->view('kpabal/dating/header',$header_data);
        $this->load->view('frontend/dating/saleDetail',$data);
        $this->load->view('kpabal/common/footer',$footer_data);
        $this->load->view('frontend/dating/result_js');
    }
    public function new_rent($id = "")
    {
        ...
        $data['key']=$this->dating_model->get_gmapi();
        $data['p_cats']=$this->dating_model->get_cat(0);
        $data['s_cats']=$this->dating_model->get_cat(3);
        //$data['j_types']=$this->dating_model->get_type();
        $data['locations']=$this->dating_model->get_locations();

        $this->load->view('kpabal/dating/header',$header_data);
        $this->load->view('frontend/dating/new_rent',$data);
        $this->load->view('kpabal/common/footer',$footer_data);
        $this->load->view('frontend/dating/result_js');
    }
    public function post_insert(){
        ...
        $this->dating_model->edit_rent($data);
        redirect("dating");
    }
    public function new_sale()
    {
        ...
        $data['key']=$this->dating_model->get_gmapi();
        $data['p_cats']=$this->dating_model->get_cat(0);
        $data['s_cats']=$this->dating_model->get_cat(9);
        $data['locations']=$this->dating_model->get_locations();

        $this->load->view('kpabal/dating/header',$header_data);
        $this->load->view('frontend/dating/new_sale',$data);
        $this->load->view('kpabal/common/footer',$footer_data);
    }
    ...
    public function edit_rent($id=0)
    {
        ...
        $this->load->view('kpabal/dating/header',$header_data);
        $this->load->view('frontend/dating/edit_rent',$data);
        $this->load->view('kpabal/common/footer',$footer_data);
    }
    ...
    public function edit_sale($id=0)
    {
        ...
        $data['key']=$this->dating_model->get_gmapi();
        $data['s_cats']=$this->dating_model->get_cat(0);
        //$data['j_types']=$this->dating_model->get_type();
        $data['locations']=$this->dating_model->get_locations();
        ...
        $this->load->view('kpabal/dating/header',$header_data);
        $this->load->view('frontend/dating/edit_sale',$data);
        $this->load->view('kpabal/common/footer',$footer_data);
    }
    ...
    public function delete_rent($id = 0){
        $this->dating_model->delete_rent($id);
        redirect("dating");
    }

    public function delete_sale($id = 0){
        $this->dating_model->delete_sale($id);
        redirect("dating");
    }
    public function author($memberIdx=0){
        ...
        $data['rents']=$this->dating_model->get_rentByAuthor($memberIdx);
        $data['sales']=$this->dating_model->get_saleByAuthor($memberIdx);

        $footer_data = $this->__generate_footer_data();
        $footer_data['additional_js'] = [
        ];
        $this->load->view('kpabal/dating/header',$header_data);
        $this->load->view('frontend/dating/author',$data);
        $this->load->view('kpabal/common/footer',$footer_data);
    ...
    public function import_data(){
        ...
        foreach($xmls as $x){
            ...
            if($email!="") {
                ...
                $author2=$author1[0];
                $author=$this->dating_model->get1_user_id($author2,$email);
                ...
                $subcat=$this->dating_model->get1_cat($subcat0);
                $region0 = $x->Cell[6]->Data[0];
                $address = $x->Cell[17]->Data[0];
                $zip0=explode("(",$address);
                $zip=explode(")",$zip0[1]);
                $region=$this->dating_model->get1_location($region0,$zip[0]);
                echo $region."<br>";
