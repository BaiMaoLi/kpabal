...
<div class="m-grid__item m-grid__item--fluid m-wrapper">
    ...
    <div class="m-subheader ">
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                <h3 class="m-subheader__title m-subheader__title--separator">Comments</h3>
            </div>
        </div>
    </div>
            ...
            <div class="m-portlet__body">
                ...
                <table class="table table-striped- table-bordered table-hover table-checkable job_table">
                    ...
                    <?php foreach($results as $details) {?>
                        <tr>
                            <td><?=$details['id']?></td>
                            <td><?=$details['comment_title']?></td>
                            <td><?=$details['author_data']['user_id']?></td>
                            <td>
                                <a href="<?=ROOTPATH?><?=ADMIN_PUBLIC_DIR?>/talky/edit_comment/<?=$details['id']?>" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="Update Detail">
                                    <i class="la la-edit"></i>
                                </a>
                                <a href="<?=ROOTPATH?><?=ADMIN_PUBLIC_DIR?>/talky/delete_comment/<?=$details['id']?>" class="btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill">
