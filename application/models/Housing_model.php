...
class Housing_model extends CI_Model {
    ...
    public function get_recent_rent($id=""){
        ...
        foreach($search as $sch){
            $loc_id=$sch['location'];
            $sch['location']=$this->getData('tbl_housing_locations',array('id'=>$loc_id))[0];
            $sch['cat']=$this->getData('tbl_housing_cat',array('id'=>$sch['category']))[0];
            $sch['subcat']=$this->getData('tbl_housing_cat',array('id'=>$sch['subcat']))[0];
            $sch['author_data']=$this->getData('tbl_member',array('memberIdx'=>$sch['author']))[0];
           $pics= json_decode($sch['images']);		
				$allpic=array();		
				foreach($pics->images as $im){		
				$allpic[]=$this->getData('tbl_housing_images',array('id'=>$im))[0]['src'];		
				}			
				$sch['images']=$allpic;
    ...
    public function edit_rent($data){
      					//$res = $this->db->insert('housing');
       /* if($data['id']!="") {
            $this->db->where("id",$data['id']);
            $res = $this->db->update('housing');
        }else{
    ...
    public function edit_sale($data){
        ...
        if($data['id']!="") {
            $this->db->where("id",$data['id']);
            $res = $this->db->update('housing_sale');
        }else{
            $res = $this->db->insert('housing_sale');
            //print_r($data);
        }
    }
    ...
    public function delete_sale($id){
        $this->db->where('id', $id);
        $this->db->delete('housing_sale');
    }
    public function get_bocker(){
        $table=$this->db->dbprefix('housing');
    ...
    public function get1_location($name,$zip){
        ...
        if($res->num_rows()>0){
            ...
            $this->db->set("name ",$name);
            $this->db->set("zip",$zip);
            $this->db->insert('housing_locations');
        }
        return $id;
    }
}
